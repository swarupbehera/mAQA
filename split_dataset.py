...
def split_data_into_binary_and_single_word_answers(csv_file, split):
    ...
    with open('dataset/metadata/single_word_{}.csv'.format(split), 'w') as f:
        data_single_word.to_csv(f, index=False)
    
    with open('dataset/metadata/binary_{}.csv'.format(split), 'w') as f:
        data_binary.to_csv(f, index=False)


csv_file_train = 'dataset/metadata/clotho_aqa_train.csv'
csv_file_val = 'dataset/metadata/clotho_aqa_val.csv'
csv_file_test = 'dataset/metadata/clotho_aqa_test.csv'
...
csv_file = 'dataset/metadata/single_word_train.csv'
csv_data = pd.read_csv(csv_file, encoding='latin1', usecols=['file_name', 'QuestionText', 'answer'])
...
with open("dataset/metadata/output_classes.json", "w") as outfile:
